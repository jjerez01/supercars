...
import es.ucm.tp1.supercars.logic.gameobjects.examenGO;
...
public class GameObjectGenerator {
	...
	public static void generateGameObjects(Game game, Level level) {
		...
		for (int x = game.getVisibility() / 2; x < game.getRoadLength(); x++) {
			...
			if (level.hasAdvancedObjects()) {
				...
				game.tryToAddObject(new examenGO(game, x, game.getRandomLane()), level.getAdvancedObjectsFrequency());
			}
	...
	public static void forceAdvanceObject(Game game, int id, int x) {
		...
		break;
		case 6:
			o = new examenGO(game, x, game.getRandomLane());
		break;
		}
